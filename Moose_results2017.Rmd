...
## Results
Gillnets were soaked for a total of `r round(sum(Table1.moose[,"Soak Time (hrs)"]),2)` hours and of `r sum(Table1.moose$n)` rainbow trout were caught. The CPUE was `r round(sum(Table1.moose$n)/sum(Table1.moose[,"Soak Time (hrs)"]),2)` fish per net-hour and was approximately equal between the floating and sinking nets (Table 1). 
There was no significant difference between the 2013 and 2017 logFL~logM relationship (p=`r round(car::Anova(fit2.rb)[3,4],3)`).
...
```{r Figure5, echo=F, results='asis', fig.height=4, fig.width=6, fig.cap = "Figure 5: Length at age with Von Bertallanfy growth curve for catch in June 2013 and July 2017. Circles are measured ages and triangles are ages predicted from an age-length key."}
